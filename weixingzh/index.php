		...
		if (!empty($postStr)){
                ...
                switch ($RX_type) {
                  ...
                  case 'shortvideo':
                    ...
                    break;
                  case 'location':
                   $result = $this -> receiveLocayion($postObj);
                    break;
                  case 'link':
...
private function receiveEvent($postObj)
{
   ...
   private function receiveText($postObj)
   {
     ...
     } elseif (strstr($content,"美女")) {
       $contentStr = array();
       $contentStr[]= array("mediaid"=>"JIREE3_FknxQ2nIrcNPxVdXK-Jj2IUgmW7Al-vcKtoM");
     ...
     elseif (strstr($content,"最新消息")) {
      ...
      if(isset($contentStr[0]['picurl']))
       ...
       } elseif (isset($contentStr[0]['mediaid'])) {
     ...
     private function xmlImage($postObj, $contentStr)
       {
          ...
          foreach($contentStr as $vo)
           $item_str .= sprintf($itemTpl, $vo['mediaid']);
...
private function weatherCate($weather,$city)
{
  ...
  {
    $contentStr[] = array("title"=>"{$city}未来五天的天气消息  ","description"=>"","picurl"=>"","url"=>"");
    $contentStr[] = array("title"=>"{$weather['data']['forecast']['0']['date']} {$weather['data']['forecast']['0']['type']} {$weather['data']['forecast']['0']['high']} {$weather['data']['forecast']['0']['low']} 空气质量指数：{$weather['data']['forecast']['0']['aqi']} 小建议：{$weather['data']['forecast']['0']['notice']}","description"=>"","picurl"=>"","url"=>"" );
    $contentStr[] = array("title"=>"{$weather['data']['forecast']['1']['date']} {$weather['data']['forecast']['1']['type']} {$weather['data']['forecast']['1']['high']} {$weather['data']['forecast']['1']['low']} 空气质量指数：{$weather['data']['forecast']['1']['aqi']} 小建议：{$weather['data']['forecast']['1']['notice']}","description"=>"","picurl"=>"","url"=>"");
    $contentStr[] = array("title"=>"{$weather['data']['forecast']['2']['date']} {$weather['data']['forecast']['2']['type']} {$weather['data']['forecast']['2']['high']} {$weather['data']['forecast']['2']['low']} 空气质量指数：{$weather['data']['forecast']['2']['aqi']} 小建议：{$weather['data']['forecast']['2']['notice']}","description"=>"","picurl"=>"","url"=>"");
    $contentStr[] = array("title"=>"{$weather['data']['forecast']['3']['date']} {$weather['data']['forecast']['3']['type']} {$weather['data']['forecast']['3']['high']} {$weather['data']['forecast']['3']['low']} 空气质量指数：{$weather['data']['forecast']['3']['aqi']} 小建议：{$weather['data']['forecast']['3']['notice']}","description"=>"","picurl"=>"","url"=>"");
    $contentStr[] = array("title"=>"{$weather['data']['forecast']['4']['date']} {$weather['data']['forecast']['4']['type']} {$weather['data']['forecast']['4']['high']} {$weather['data']['forecast']['4']['low']} 空气质量指数：{$weather['data']['forecast']['4']['aqi']} 小建议：{$weather['data']['forecast']['4']['notice']}","description"=>"","picurl"=>"","url"=>"");
