...
if(isset($_POST['dosubmit']))
{
  ...
  if($upFile->upload('up_file'))
  {
    ...
    //把数据保存到数据库
    $time = time();
    $sql = "INSERT INTO `meida`(`name`, `type`, `media_id`, `createTime`) VALUES ('{$filename}','{$_POST['file_type']}','{$data['media_id']}',$time)";
    $query = mysql_query($sql);
    if($query)
    {
      echo '<script>alert("上传文件成功~~");location.href="/up_file/file_list.php";</script>';
    } else {
